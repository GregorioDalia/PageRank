...
int main(int argc, char *argv[]){
  ...
  float teleport_probability = 0;           /* probability of the random walker to teleport on a random node */ 
  ...
  if(rank == MASTER){ 
    ...
    if(n < numtasks){
      fprintf(stderr, "[Error] too many processes for the number of the nodes");
      exit(1);
    } 
  ...
  local_sub_page_ranks = malloc((rows_num + 2) * sizeof(float));
