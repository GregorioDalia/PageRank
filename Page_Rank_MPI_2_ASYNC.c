    ...
    */
    ...
    printf("SCORE NORM A QUSTO GIRO = %f\n",score_norm);
      */
      ...
      for(int i=1; i<numtasks; i++){
        MPI_Isend(complete_page_ranks, n+1, MPI_FLOAT, i, TAG, MPI_COMM_WORLD,&request);
    ...
    // WORKERS send the new page_rank values
      ...
      MPI_Isend(local_sub_page_ranks, rows_num+1, MPI_FLOAT, 0, TAG, MPI_COMM_WORLD,&request);
